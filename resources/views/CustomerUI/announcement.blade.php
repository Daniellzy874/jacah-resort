<div class="w-[100%] top-[95%] left-[50%] translate-x-[-50%] translate-y-[-50%] pointer-none fixed ">
    <h1 class="nav__link text-xl mt-2 ">
        <img id="content_toggler" src="{{asset('assets/image/announce.png')}}" class="h-[100px] w-[100px] rounded-full shadow-md absolute bottom-[0] cursor-pointer" alt="">
...
<div id="content_container" class="content_container">
    <div class="container-section" style="border-radius:10px; padding:10px; height:100%;">
        <main id="promo-content" class="h-[100%] flex">
            <img id="promo-image" src="{{asset('assets/image/announcement/announcement.png')}}" class="w-[50%]" alt="">
            <div class="w-[50%] flex flex-col items-center justify-center gap-4">
                ...
                @else
                <a href="{{route('login')}}" class="bg-green-400 hover:bg-green-500 py-4 px-8 text-white text-[30px] rounded-md cursor-pointer">Sign in</a>
                @endauth
                @endif
            </div>
        </main>
...
<script>
    ...
    fetch('http://127.0.0.1:8000/promo_json')
        ...
        .then(data => {
            data.map((datas) => {
                document.getElementById('promo-image').src = `{{asset('assets/image/announcement/${datas.photo}')}}`;
